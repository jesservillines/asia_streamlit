...
# Target columns - using exact muscle names
TARGET_COLS = [
...
def load_data_and_models():
    ...
    # Get common PIDs
    common_pids = set(train_features['pid'].unique()) & \
                  set(train_processed['pid'].unique()) & \
                  set(week26_outcomes['pid'].unique())
    common_pids = sorted(list(common_pids))
    print(f"Found {len(common_pids)} patients with complete data")
    
    # Filter and align data
    train_features = train_features[train_features['pid'].isin(common_pids)].sort_values('pid').reset_index(drop=True)
    train_processed = train_processed[train_processed['pid'].isin(common_pids)].sort_values('pid').reset_index(drop=True)
    week26_outcomes = week26_outcomes[week26_outcomes['pid'].isin(common_pids)].sort_values('pid').reset_index(drop=True)
    
    # Prepare features
    feature_cols = [col for col in train_processed.columns if col != 'pid']
    X = train_processed[feature_cols].values
    
    # Prepare targets (averaging left and right)
    y_true = []
...
def compute_shap_values(models, X, feature_names, sample_size=100):
    ...
    for name, model in models.items():
        ...
        if name == 'xgb':
            # For XGBoost MultiOutputRegressor, we need to handle each output
            shap_vals_list = []
            base_vals_list = []
            
            # Get the first target to understand the structure
...
def calculate_ensemble_shap_with_uncertainty(shap_values_dict, weights):
    ...
    # First, calculate the mean (which is our ensemble)
...
def main():
    ...
    try:
        ...
        X, y_true, feature_names, models, pids = load_data_and_models()
